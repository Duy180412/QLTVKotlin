package com.example.qltvkotlin.presentation.helper.action
...
interface TakePhotoActionOwner {
    val takePhotoAction: TakePhotoAction
        get() {
            return when (this) {
                is Activity -> TakePhotoAction(
                    this as ActivityResultCaller
                )
                is Fragment -> TakePhotoAction(
                    requireActivity() as ActivityResultCaller
                )
                else -> error("Lỗi")
            }
        }
